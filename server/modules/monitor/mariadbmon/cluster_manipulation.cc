...
MariaDBMonitor::failover_prepare(FailoverType fo_type, Log log_mode, OpStart start, mxb::Json& error_out)
{
    ...
    // If replacing a hanged master, ignore demotion checks as they do not apply.
    else if (fo_type != FailoverType::WRITES_HANGED
    ...
    if (demotion_target)
    {
        ...
        if (fo_type == FailoverType::ALLOW_TRX_LOSS || fo_type == FailoverType::WRITES_HANGED)
        {
                ...
                else
                {
                    ...
                    if (m_settings.write_test_fail_action == WriteTestFailAction::FAILOVER)
                    {
                        if (m_write_test_fails >= m_settings.failcount)
                        {
                            ...
                            mxb::Json dummy(mxb::Json::Type::UNDEFINED);
                            auto op = failover_prepare(FailoverType::WRITES_HANGED, log_mode, OpStart::AUTO,
                                                       dummy);
                            if (op)
                            {
